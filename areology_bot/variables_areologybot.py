...
class GlobalVariables:
    def __init__(self):
        ...
        self.nearby_enemey_buildings = False

        self.larvae = self.drones = self.queens = self.overlords = None # Macro Units

        self.zerglings = self.roaches = self.hydralisks = \
        self.overseers = self.corrupters = self.broodlords = None # Army Units

        self.hatcheries = self.lairs = self.hives = self.extractors = \
        self.spawning_pools = self.roach_warrens = self.hydralisk_dens = \
        self.evolution_chambers = self.infestation_pits = self.spires = \
        self.greater_spires = self.spore_crawlers = self.spine_crawlers = None # Structures

        self.furthest_townhall_from_map_center = None # Furthest Townhall (used for building placement)

        self.burrowed_drones = [], self.burrowed_queens = [], self.burrowed_zerglings = [], \
        self.burrowed_roaches = [], self.burrowed_hydralisks = [] # Burrowed Units

        self.enemy_units = self.ground_enemey_units = self.flying_enemy_units = None # Enemy Units
        self.enemy_structures = self.flying_enemy_structures = None # Enemy Structures
